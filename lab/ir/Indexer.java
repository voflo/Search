...
public class Indexer {
	...
	//public Index index = new OnFileIndex();
	...
	public void processFiles( File f ) {
		...
		if ( f.canRead() ) {
			if ( f.isDirectory() ) {
				...
				if ( docID%100 == 0 ) System.err.println( "Indexed " + docID + " files" );
	...
	/**
	 * Check that all lists in the index are sorted.
	 * 
	 * @return True if the lists are sorted, false otherwise
	 */
	public boolean checkSanity(){
		...
		while(it.hasNext()){
			int lastDocID = -1;
			PostingsList pl = index.getPostings(it.next());
			if (pl.size() > 0){
				for(int i = 0; i < pl.size(); i++){
					PostingsEntry pe = pl.get(i);
					if(pe.docID > lastDocID){ //check that docIDs are sorted!
						lastDocID = pe.docID;
					} else {
						System.err.println("docID:s not sorted!");
						return false;
					}
					
					if(!pe.offsets.isEmpty()){
						int lastOffset = -1;
						for (int j = 0; j < pe.offsets.size(); j++){
							if(pe.offsets.get(j) >= lastOffset){ //check that offsets are sorted!
								lastOffset = pe.offsets.get(j);
							} else {
								System.err.println("Offsets not sorted!");
								System.err.println(pe.offsets);
								return false;
							}
						}
					} else{
						System.err.println("Offset-list was empty!!??!");
						return false;
					}
				}
			} else{
				System.err.println("PostingsList was empty?!?!?!");
				return false;
			}
		}
		System.err.println("Sanity is good!");
		return ret;
	}
